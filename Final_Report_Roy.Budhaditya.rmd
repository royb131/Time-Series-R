...
summary(globtemp)
```
MAD is slightly less than the standard deviation(SD). This is one reason not to use the MAD for normal distributions.
...
```
#Data description
After plotting the data it appreas to behave like a random walk than a trend stationary series. In a random walm, current observation is equal to the previous observation with a random step up or down.
...
Further chec of stationary and non- stationary.If stationary, the ACF/PACF plots will show a quick drop-off in correlation after a small amount of lag between points.

```{r}
acf2(gtemp)
```
Clearly this data is non-stationary as a high number of previous observations are correlated with future values.
...
Noises are wide in ACF since most of the distrobutions are not in blue line created by ACF plot. This clearly shows it is not a white noise and there is info in the data which can be used to forecast future values. Here PACF is lamost 1 at lag 1 . 
R plots 95% significance boundaries as blue dotted lines. There are significant autocorrelations with many lags in our data, as shown by the ACF plot above However, this could be due to carry over correlation from the first or early lags, since the PACF plot only shows a spike at lags 1 and 2.
